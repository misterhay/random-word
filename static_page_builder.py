...
<div style="font-size:200%" id="titleDiv">{title}</div>
<p> </p>
<div id="buttonDiv">
...
for grade in fry_lists:
    title = 'Word Game ' + grade
    list = fry_lists[grade]
    with open(grade + '.html', 'w') as f:
        html_string = html_template.format(title=title, list=list)
        f.write(html_string)
        f.close()
